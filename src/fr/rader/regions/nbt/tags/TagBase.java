package fr.rader.regions.nbt.tags;
...
public abstract class TagBase {
    ...
    public TagList<TagCompound> getAsCompoundList() {
        ...
        if(list.getChildrenID() == 0) {
            return new TagList<>(TagCompound.class, list.getName());
        }

        if(list.getChildrenID() != TagCompound.TAG_ID) {
            throw new RuntimeException("Cannot cast to TagList<TagCompound> because the childen ID does not match the TagCompound ID (Got " + list.getChildrenID() + " instead of " + TagCompound.TAG_ID +")");
        }
    ...
    public TagList<?> getAsUnknownList() {
        if(!(this instanceof TagList)) {
